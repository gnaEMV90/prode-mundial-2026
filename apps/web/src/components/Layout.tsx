      ...
      <header className="sticky top-0 z-20 border-b border-white/10 bg-slate-950/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
          ...
          <nav className="hidden items-center gap-4 md:flex">
            {navItems.map((item) => (
              <DesktopNavLink key={item.to} to={item.to}>{item.label}</DesktopNavLink>
            ))}
            ...
            {user?.role === 'ADMIN' && <NavLink to="/admin" className="text-sm text-amber-300 hover:text-amber-200">Admin</NavLink>}
          </nav>
          <div className="flex items-center gap-2">
            {user ? (
              <>
                <span className="hidden text-sm text-slate-300 sm:block">{user.name}</span>
                <button onClick={handleLogout} className="rounded-xl border border-white/10 px-3 py-2 text-sm text-slate-200 hover:bg-white/10">Salir</button>
              </>
            ) : (
              <>
                <Link to="/login" className="rounded-xl border border-white/10 px-3 py-2 text-sm text-slate-200 hover:bg-white/10">Entrar</Link>
                <Link to="/registro" className="rounded-xl bg-emerald-400 px-3 py-2 text-sm font-bold text-slate-950 hover:bg-emerald-300">Registrarme</Link>
              </>
            )}
          </div>
        </div>
        <nav className="mx-auto flex max-w-6xl gap-3 overflow-x-auto px-4 pb-3 md:hidden">
          {navItems.map((item) => (
            <MobileNavLink key={item.to} to={item.to}>{item.label}</MobileNavLink>
          ))}
          ...
          {user?.role === 'ADMIN' && <NavLink to="/admin" className="whitespace-nowrap rounded-full bg-amber-400 px-3 py-1 text-sm font-bold text-slate-950">Admin</NavLink>}
        </nav>
      </header>
...
function MobileNavLink({ to, children }: { to: string; children: string }) {
  return (
    <NavLink
      to={to}
      className={({ isActive }) =>
        `whitespace-nowrap rounded-full px-3 py-1 text-sm ${isActive ? 'bg-emerald-400 text-slate-950' : 'bg-white/10 text-slate-200'}`
      }
    >
      {children}
    </NavLink>
