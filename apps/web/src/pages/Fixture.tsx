    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-white/10 bg-white/10 p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-300 sm:text-base">
          Consultá partidos, fechas, sedes y resultados del Mundial 2026. Los horarios son la referencia para bloquear
          pronósticos.
        </p>
      </section>
...
export function MatchCard({ match, children }: { match: Match; children?: ReactNode }) {
  ...
  return (
    <article className="rounded-3xl border border-white/10 bg-white/10 p-4 shadow-lg shadow-black/10">
...
function StatusBadge({ status }: { status: Match['status'] }) {
  ...
  return <span className={`rounded-full px-3 py-1 text-xs font-black uppercase tracking-wide ${config.className}`}>{config.label}</span>;
}